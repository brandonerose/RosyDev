dev_combine_split_R_files <- function(choice = "combine",
                                      ...
                                      overwrite = FALSE) {
  if (length(choice) > 1)
    stop("Choice must be length 1")
  choices <- c("split", "combine", "both")
  if (!choice %in% choices)
    stop("Choice must be one of... ", as_comma_string(choices))
...
#' @return message
#' @export
combine_R_files <- function(source_dir = file.path(getwd(), "R"),
                            destination_dir = file.path(getwd(), "dev"),
                            file_name = "combined",
                            file_ext = ".R",
                            header_symbol = "=",
                            max_new_lines = 0,
  ...
  if (!file_ext %in% c(".R", ".Rmd"))
    stop("file_ext must be R or Rmd")
  expected_folder <- file.path(source_dir)
  if (!file.exists(expected_folder)) {
    if (!silent)
      cli_alert_wrap("No folder", file = expected_folder)
    return(invisible())
  }
  ...
  if (!file.exists(destination_file) || overwrite) {
    dir.create(destination_dir,
               showWarnings = FALSE,
               recursive = TRUE)
    file_list <- list.files(source_dir,
                            pattern = paste0("\\", file_ext, "$"),
                            full.names = TRUE)
    combined_text <- character(0)
    for (file in file_list) {
      # file <- file_list %>% sample(1)
    ...
    if (!silent)
      cli_alert_wrap("Combined file saved to:",
...
#' @return message
#' @export
split_R_files <- function(source_dir = file.path(getwd(), "dev"),
                          destination_dir = file.path(getwd(), "R"),
                          file_name = "combined",
                          file_ext = ".R",
                          header_symbol = "=",
                          new_lines = character(0),
  ...
  if (anyDuplicated(script_names))
    stop(
      "You have duplicate script names! --> ",
      as_comma_string(RosyUtils::vec_which_duplicated(script_names))
    )
  ...
  for (i in seq_along(scripts)) {
    ...
    writeLines(new_lines %>% append(scripts[[i]]) %>% paste0(collapse = "\n"),
               con = output_file)
...
#' @return message
#' @export
pkg_combine_R_files <- function(pkgs,
  ...
  if (file_name_type == "name__version") {
    installed_packages$comparison_name <- paste0(
      installed_packages$Package,
      "__",
      gsub("\\.|\\-", "_", installed_packages$Version)
    )
  }
  ...
  if (!file.exists(destination_dir)) {
    if (utils::menu(
      c("Yes", "No and stop"),
      title = paste0(
        "Your listed directory does not exist! Would you like to create? ",
        destination_dir
      )
    ) == 2) {
      stop("Stopped!")
  ...
  if (length(BAD) > 0)
    stop("You have listed pkgs that are not installed: ",
         BAD %>% paste0(collapse = ", "))
  ...
  if (length(pkgs_there) > 0)
    message("Packages already there: ",
            pkgs_there %>% paste0(collapse = ", "))
  if (length(pkgs_missing) > 0)
    message("Packages missing: ", pkgs_missing %>% paste0(collapse = ", "))
  if (!overwrite) {
    if (file_name_type == "name__version") {
      downloaded_file_list <-
        downloaded_file_list %>%
  ...
  if (length(pkgs_missing) == 0)
    message("Nothing to be done at... ", destination_dir)
  ...
  for (pkg in pkgs) {
    ...
    if (pkg %in% pkgs_missing) {
      ...
      only_missing_R_folder <- file.exists(source_dir_root) &&
        !file.exists(source_dir)
      if ((overwrite ||
           !file.exists(source_dir)) && !only_missing_R_folder) {
  ...
  if (length(pkgs_missing) > 0)
    message("Packages STILL missing: ",
            pkgs_missing %>% paste0(collapse = ", "))
}
...
#' @return message
#' @export
pkg_combine_R_files_launch <- function(pkg,
